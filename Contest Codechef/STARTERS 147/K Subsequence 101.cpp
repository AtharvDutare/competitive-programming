...
*/
void solve(vi &arr,int index,int k,int &ans,int tempAns) {
    if(index>=arr.size()) {
        if(k==0) {
            ans=max(tempAns,ans);
        }
    ...
    if(k>1&&tempAns==0) {
        solve(arr,index+1,k-1,ans,tempAns+arr[index]);
    }
    else if(k>1){
        solve(arr,index+1,k-1,ans,tempAns+2*arr[index]);
    }
    else if(k==1) {
        solve(arr,index+1,k-1,ans,tempAns+arr[index]);
    }

    solve(arr,index+1,k,ans,tempAns);
}
/* clang-format on */
void test_case()
{
