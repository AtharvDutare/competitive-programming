...
*/
// int lis(vll &arr,int index,int prev){
//     if(index>=arr.size()){
//         return 0;
//     }
//     int include=0;
//     if(prev==-1||(index-prev==1&&arr[index]>arr[prev]))
//     include=lis(arr,index+1,index)+1;
//     int exclude=lis(arr,index+1,-1);
...
void test_case()
{
    ...
    while(i<n){
        int tempAns=0;
        while(tempAns==0||(i<n&&arr[i]>arr[i-1])){
            tempAns++;
            i++;
        }
        //cout<<tempAns<<" "<<i<<endl;
