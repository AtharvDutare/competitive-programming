...
/* clang-format on */
void solve(vector<vector<ll>>& mat,int n,int m){
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            int temp=mat[i][j];
            int left=-1;
            ...
            //cout<<left<<" "<<right<<" "<<top<<" "<<bottem<<" "<<temp<<endl;
            if((left<temp)&&(right<temp)&&(top<temp)&&(bottem<temp)){
                //yes();
                int t=max(left,max(right,max(top,bottem)));
                mat[i][j]=t;
                solve(mat,n,m);
                return ;
            }
