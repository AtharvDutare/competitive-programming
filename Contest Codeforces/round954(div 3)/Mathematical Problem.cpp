...
/* clang-format on */
ll solve(string s,int start,int end,int count){
    ...
    for(int i=start;i<end;i++){
        ...
        string rights;
        for(int w=i;w<end;w++){
            rights.push_back(s[w]);
        }
        ll tempAns;
        ll left=solve(lefts,start,i,count-1);
        ll right=solve(rights,i+1,end,count-1);
        if(left*right>left+right){
            tempAns=left+right;
        }
        else{
